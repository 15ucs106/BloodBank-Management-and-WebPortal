...
<div class="wrap">
		<ul class="nav">
			...
			<li><a href="blooddonated.php">Update Blood</a></li>
...
<tr><td class="lefttd">Select donor </td><td><select name="s4" required><option value="">Select</option>
<?php
$cn=makeconnection();
$s="select * from donarregistration";
	...
	{
		if(isset($_POST["show"])&& $data[0]==$_POST["s4"])
		{
			echo "<option value=$data[6] selected>$data[1]</option>";
		}
		else
		{
			echo "<option value=$data[6]>$data[1]</option>";
		}
...
if(isset($_POST["sbmt"])) 
{
	...
	//echo $d;
$cn=makeconnection();
			$s="insert into donation(camp_id,ddate,units,detail,email) values('" . $_POST["s3"] . "','". $d ."' ,'" . $_POST["t3"] . "','" . $_POST["t4"] . "','". $_POST["s4"] ."')";
